...
class Mask2FormerVideoHeadTube(AnchorFreeHead):
    ...
    def forward(self, feats, img_metas, return_query=False):
        ...
        mask_features, multi_scale_memorys = self.pixel_decoder(feats, num_frame) # feats: list of multi-scale feature: each shape is (b*T, c, h, w)
        ...
        if return_query:
            return cls_pred_list, mask_pred_list, query_feat.permute(1,0,2).contiguous()
    ...
    def simple_test_with_query(self, feats, img_metas, **kwargs):
        ...
        Returns:
            ...
            - mask_pred_results (Tensor): Mask logits, shape \
                (batch_size, num_queries, h, w).
        """
        all_cls_scores, all_mask_preds, query_feat = self.forward(feats, img_metas, return_query=True)
        ...
        # upsample masks
        img_shape = img_metas[0][0]['batch_input_shape']
        # print('what is the shape you are upsampling here', img_shape)
        bs, nf = mask_pred_results.size()[:2]
        mask_pred_results = F.interpolate(
            mask_pred_results.flatten(0, 1),
            size=(img_shape[0], img_shape[1]),
