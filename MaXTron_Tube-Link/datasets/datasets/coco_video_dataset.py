...
from datasets.datasets.core import CocoVID
...
class CocoVideoDataset(CocoDataset):
    ...
    def load_video_anns(self, ann_file):
        ...
        self.inds_when_all_frames = [] if self.load_all_frames else None
        cnt = 0
        for vid_id in self.vid_ids:
            ...
            if self.load_all_frames:
                ...
                self.inds_when_all_frames.append(cnt)
                cnt += len(info) - 1
            else:
    ...
    def evaluate(self,
        ...
        eval_results = dict()
        if 'track' in metrics:
            raise NotImplementedError("HY : we delete eval_mot to get rid of extra package")
