...
@torch.no_grad()
def compute_mask_similarity(inputs: torch.Tensor, targets: torch.Tensor, masking_void_pixel=True):
    """
    ...
    inputs = inputs.flatten(1) # N x THW

    pixel_gt_non_void_mask = (targets.sum(0, keepdim=True) > 0).to(inputs) # 1xTHW
...
class VideoHungarianMatcher(nn.Module):
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            out_mask = outputs["pred_masks"][b].flatten(1)  # [num_queries, T_pred, H_pred, W_pred] -> [num_queries, THW]
            # gt masks are already padded when preparing target
            tgt_mask = targets[b]["masks"].to(out_mask).flatten(1) # [num_queries, T, H_pred, W_pred] -> [num_queries, THW]
            with autocast(enabled=False):
                mask_similarity = compute_mask_similarity(out_mask.float(), tgt_mask.float(), self.masking_void_pixel)
