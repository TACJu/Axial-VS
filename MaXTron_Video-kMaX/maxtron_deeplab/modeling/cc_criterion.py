...
def pixelwise_insdis_loss(
    ...
    # pixel_feature: B x C x T x H x W
    # gt_mask: B x N x T x H x W
    pixel_feature = pixel_feature.flatten(2) # B x C x THW
    gt_mask = gt_mask.flatten(2) # B x N x THW
    pixel_gt_void_mask = pixel_gt_void_mask.flatten(1) # B x THW
    inverse_gt_mask_area = inverse_gt_mask_area.flatten(1) # B x THW

    sample_logits = torch.log(inverse_gt_mask_area) * sample_temperature # B x THW
...
def aux_semantic_loss(
    ...
    pred_semantic_logits = pred_semantic_logits.flatten(2) # B x C x THW
    ground_truth_semantic = ground_truth_semantic.flatten(1) # B x THW
    pixel_gt_void_mask = pixel_gt_void_mask.flatten(1) # B x THW
    inverse_gt_mask_area = inverse_gt_mask_area.flatten(1) # B x THW
...
def dice_loss(
    ...
    """
    inputs = inputs.softmax(1) # B N THW
...
def softmax_ce_loss(
    ...
    Returns:
        Loss tensor
    """
    loss = F.cross_entropy(inputs, targets, reduction="none") # B x THW
    if masking_void_pixel:
...
class MaXTronCCSetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses, share_final_matching,
        ...
        Parameters:
            ...
            instance_classes: get instance classes of the class
        """
        super().__init__()
    ...
    def loss_masks(self, outputs, targets):
        ...
        """
        src_masks = outputs["pred_masks"] # B x N x T x H x W
        target_masks = targets["masks"] # B x N x T x H x W
        pq_loss_mask_weight = targets["pq_loss_mask_weight"] # B x N
        pixel_gt_void_mask = targets["pixel_gt_void_mask"] # B x T x H x W 

        src_masks = src_masks.flatten(2) # B x N x THW
        target_masks = target_masks.flatten(2) # B x N x THW
        pixel_gt_void_mask = pixel_gt_void_mask.flatten(1) # B x THW
    ...
    def process_gt(self, outputs, targets, indices, matched_dice, matched_cls_prob, process_semantic=False):
        ...
        src_masks = outputs["pred_masks"].detach() # B x N x T x H x W

        # Pad and permute the target_mask B x N x T x H x W
        ...
        pixel_gt_void_mask = (target_masks.sum(1) < 1) # B x T x H x W
   
        # inverse_gt_mask_area is used to sample pixels.
        mask_gt_area = target_masks.sum(2).sum(2).sum(2) # B x N
        pixel_gt_area = torch.einsum('bnthw,bn->bthw', target_masks, mask_gt_area) # B x T x H x W
        inverse_gt_mask_area = (pixel_gt_area.shape[1] * pixel_gt_area.shape[2] * pixel_gt_area.shape[3]) / torch.clamp(pixel_gt_area, min=1.0) # B x T x H x W
        ...
        void_mask = pixel_gt_void_mask.to(src_masks_prob) # B x T x H x W
        ...
        processed_gt = {"masks": target_masks, "labels": target_classes,
            "pq_loss_mask_weight": matched_cls_prob, # B x N
            "pq_loss_class_weight": matched_dice, # B x N
            "pixel_gt_void_mask": pixel_gt_void_mask, # B x T x H x W
            "inverse_gt_mask_area": inverse_gt_mask_area,} # B x T x H x W

        if process_semantic:
            # To obtain semantic gt
            ground_truth_semantic = [t["semantic_masks"] for t in targets]
            ground_truth_semantic = torch.stack(ground_truth_semantic, dim=0) # B x T x H x W
    ...
    def forward(self, outputs, targets, clip_outputs=None):
        """This performs the loss computation.
        Parameters:
             outputs: dict of tensors, see the output specification of the model for the format
             targets: list of dicts, such that len(targets) == batch_size.
                      The expected keys in each dict depends on the losses applied, see each loss' doc
             contrastive_outputs: dict of tensors, used for IDOL contrastive training objective
        """
        if clip_outputs is None:
            outputs_without_aux = {k: v for k, v in outputs.items() if k != "aux_outputs"}
        else:
            outputs_without_aux = {k: v for k, v in clip_outputs.items() if k != "aux_outputs"}
        ...
        processed_targets = self.process_gt(outputs, targets, indices, matched_dice, matched_cls_prob, process_semantic=self.process_semantic)
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                for loss in self.losses:
                    if loss in ['aux_semantic', 'aux_contrast']:
                        # Only for final output.
                        continue
                    if loss == 'pixels' and 'pixel' not in aux_outputs:
                        continue
