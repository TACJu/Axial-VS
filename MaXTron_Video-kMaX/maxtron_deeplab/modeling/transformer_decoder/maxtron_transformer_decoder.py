...
from kmax_deeplab.modeling.transformer_decoder.kmax_transformer_decoder import TRANSFORMER_DECODER_REGISTRY
...
class kMaXPredictor(nn.Module):
    ...
    def forward(self, mask_embeddings, class_embeddings, pixel_feature):
        ...
        cluster_mask_kernel = cluster_mask_kernel.permute(0, 2, 1).contiguous() # BxCxN->BxNxC, to align with mask2former format
        ...
        return {
            ...
            'mask_embeddings': cluster_mask_kernel,
...
class kMaXTransformerLayer(nn.Module):
    def __init__(
        ...
        self._predictor = kMaXPredictor(in_channel_pixel=self._bottleneck_channels,
    ...
    def forward(self, pixel_feature, query_feature):
        N, C, TH, W = pixel_feature.shape
        _, D, L = query_feature.shape
        pixel_space = self._pixel_conv1_bn_act(F.gelu(pixel_feature)) # N C TH W
        query_space = self._query_conv1_bn_act(query_feature) # N x C x L

        # k-means cross-attention.
        pixel_value = self._pixel_v_conv_bn(pixel_space) # N C TH W
        pixel_value = pixel_value.reshape(N, self._total_value_depth, TH*W)
        # k-means assignment.
        prediction_result = self._predictor(
            mask_embeddings=query_space, class_embeddings=query_space, pixel_feature=pixel_space)
        clustering_result = prediction_result['mask_logits'].flatten(2).detach() # N L THW
...
class CrossAttentionTransformerLayer(nn.Module):
    ...
    def forward(self, pixel_feature, query_feature):
        ...
        pixel_k = pixel_k.reshape(N, self._num_heads, self._total_key_depth//self._num_heads, H*W)
        pixel_v = pixel_v.reshape(N, self._num_heads, self._total_value_depth//self._num_heads, H*W)
...
class MaXTronTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self._predictor = kMaXPredictor(in_channel_pixel=256,
        ...
        self._cross_clip_training = cross_clip_training
    ...
    def from_config(cls, cfg, input_shape_from_backbone):
        ...
        # video
        ret["num_clip_frames"] = cfg.INPUT.NUM_CLIP_FRAMES
        # cross-clip training
        ret["cross_clip_training"] = cfg.MODEL.MAXTRON.CROSS_CLIP_TRACKING_MODULE.ENABLE
        return ret


    def forward(self, x, panoptic_features, semantic_features):
        BT = x[0].shape[0]
        B = BT // self._num_clip_frames if (self.training or self._cross_clip_training) else 1
        T = BT // B

        panoptic_features = rearrange(panoptic_features, '(b t) c h w -> b c (t h) w', b=B)
        ...
        predictions_class = []
        predictions_mask = []
        predictions_mask_embedding = []
        predictions_pixel_feature = []

        for i, feat in enumerate(x):
            for _ in range(self._num_blocks[i]):
                feat_ = rearrange(feat, '(b t) c h w -> b c (t h) w', b=B)
                cluster_centers, prediction_result = self._kmax_transformer_layers[current_transformer_idx](
                    pixel_feature=feat_, query_feature=cluster_centers
                )
                predictions_class.append(prediction_result['class_logits']) # B x N x C
                predictions_mask.append(rearrange(prediction_result['mask_logits'], 'b c (t h) w -> b c t h w', t=T)) # B x C x T x H x W
                predictions_pixel_feature.append(rearrange(prediction_result['pixel_feature'], 'b c (t h) w -> b c t h w', t=T)) # B x C x T x H x W
                current_transformer_idx += 1

        class_embeddings = self._class_embedding_projection(cluster_centers)
        mask_embeddings = self._mask_embedding_projection(cluster_centers) # B x C x L
        ...
        predictions_mask.append(rearrange(prediction_result['mask_logits'], 'b n (t h) w -> b n t h w', t=T))
        predictions_mask_embedding.append(prediction_result['mask_embeddings'])
        predictions_pixel_feature.append(rearrange(prediction_result['pixel_feature'], 'b c (t h) w -> b c t h w', t=T))

        out = {
            'pred_logits': predictions_class[-1],
            'pred_masks': predictions_mask[-1],
            'pred_mask_embeddings': predictions_mask_embedding[-1],
            'pixel_feature': predictions_pixel_feature[-1],
            'aux_outputs': self._set_aux_loss(
                predictions_class, predictions_mask, predictions_pixel_feature
            ),      
            'cluster_centers': cluster_centers.permute(0,2,1).contiguous()
        }
        ...
        if self._add_aux_semantic_pred and self.training:
            ...
            if self._use_aux_semantic_decoder:
                ...
                aux_semantic_prediction = rearrange(aux_semantic_prediction, '(b t) c h w -> b c (t h) w', b=B)
            else:
    ...
    def _set_aux_loss(self, outputs_class, outputs_seg_masks, outputs_pixel_feature):
        ...
        return [
            ...
            "pixel_feature": F.interpolate(c, size=target_size, mode="trilinear", align_corners=align_corners),}
